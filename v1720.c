...
void  v1720_SetMonitorMode(MVME_INTERFACE *mvme, uint32_t base, int mode)
{
    regWrite(mvme, base, V1720_MONITOR_MODE , mode);
}

void  v1720_SetMonitorVoltageValue(MVME_INTERFACE *mvme, uint32_t base, int value)
{
    if(value > 4096)
    {
        printf("Exceeded Maximum of Analog Monitor\n");
        value = 4090;
    ...
    if(value < 0)
    {
        printf("Wrong Value for Analog Monitor\n");
    }
    if(0 <=  value <= 4096)
        regWrite(mvme, base, V1720_SET_MONITOR_DAC, value);
}
